...
export function ReportFiltersComponent({
  ...
  const handleStateChange = useCallback(
    (state: string) => {
      onFiltersChange({ ...filters, state: state === 'all' ? undefined : state, lga: undefined })
    },
    [filters, onFiltersChange]
  )
  ...
  const handleLGAChange = useCallback(
    (lga: string) => {
      onFiltersChange({ ...filters, lga: lga === 'all' ? undefined : lga })
    },
    [filters, onFiltersChange]
  )
  ...
  const handleProgramChange = useCallback(
    (programId: string) => {
      onFiltersChange({ ...filters, programId: programId === 'all' ? undefined : programId })
    },
    [filters, onFiltersChange]
  )
  ...
  const hasActiveFilters = Boolean(
    filters.state || filters.lga || filters.startDate || filters.endDate || filters.programId
  )
        ...
        {availablePrograms.length > 0 && (
          ...
          >
            <Select value={filters.programId || 'all'} onValueChange={handleProgramChange}>
              <SelectTrigger className="w-[200px] transition-all duration-200 hover:border-primary/50 hover:ring-2 hover:ring-primary/20">
